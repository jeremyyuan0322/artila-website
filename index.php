...
class Page
{
	...
	public static function getYamlFilePath($baseDir, $lang, $pageName)
	{
		...
		if (file_exists("{$langDir}/{$pageName}.yaml")) {
			// echo "yaml: ".$path . "<br>";
			return "{$langDir}/{$pageName}.yaml";
		}
		else{
			// 檢查特定子目錄
			$subDirs = ['news', 'application', 'product'];
			foreach ($subDirs as $subDir) {
				$path = "{$langDir}/{$subDir}/{$pageName}.yaml";
				if (file_exists($path)) {
					// echo "yaml: ".$path . "<br>";
					return $path;
				}
		...
		// echo "yaml: "."{$langDir}/{$pageName}.yaml" . "<br>";
		error_404();
...
// Remove the first element ('jeremy') as it's not part of the count
array_shift($parts);
